        ...
        private void MapDRETotalizer(List<AccountPlanClassification> models, List<TotalizerClassificationModel> totalizerClassifications)
        {
            foreach (var classification in models.Where(m => m.TypeClassification == ETypeClassification.DRE))
            {
                int? totalizerId = classification.Name switch
                {
                    ...
                    "(-) Custos das Mercadorias" or "(-) Custos dos Serviços Prestados" or "(-) Custos Variáveis"
            ...
            var prioridade = new List<string>
{
    ...
    int year)
        {
            ...
            if (groupPlan != null)
            {
                var dreGrupo = await BuildPainelByTypeDRE(
                    groupPlan.Id,
                    year,
                    3);

                // só usa se realmente possuir classificação
                if (PainelPossuiValores(dreGrupo))
                {
                    painelGrupo = dreGrupo;
                }
            }

            // se não tiver classificação -> retorna vazio
            if (painelGrupo == null)
            {
                painelGrupo = await CriarPainelVazio(year);
            }
        ...
        {
            // cria um plano inexistente só para obter estrutura vazia padrão
            return await BuildPainelByTypeDRE(0, year, 3);
        }
    ...
    List<PainelBalancoContabilRespone> paineisEmpresas)
            ...
            var variacao = new PainelBalancoContabilRespone
            {
                Months = realizado.Months.Select(r =>
                {
                    ...
                    var custoServicos = totalizerVar.SelectMany(t => t.Classifications)
                        .FirstOrDefault(c => c.Name == "(-) Custos dos Serviços Prestados")?.Value ?? 0;
                    var custosVariaveis = totalizerVar.SelectMany(t => t.Classifications)
                        .FirstOrDefault(c => c.Name == "(-) Custos Variáveis")?.Value ?? 0;
                    if (lucroBruto != null)
                        lucroBruto.TotalValue = (receitaLiquida?.TotalValue ?? 0) + custoMercadorias + custoServicos + custosVariaveis;
        ...
        public async Task<List<DREComparativosItemDto>> BuildDemonstracaoResultadoSimplificado(int accountPlanId, int year)
        {
            ...
            var custoVarReal = GetValoresMensaisRealizados(realizado, "(-) Custos Variáveis");
            ...
            var custoVarOrc = GetValoresMensaisOrcados(orcado, "(-) Custos Variáveis");

            // Soma mês a mês (não soma o ano inteiro)
            var custosReal = custoMercReal
                .Zip(custoServReal, (m, s) => m + s)
                .Zip(custoVarReal, (ms, v) => ms + v)
                .ToList();
            var custosOrc = custoMercOrc
                .Zip(custoServOrc, (m, s) => m + s)
                .Zip(custoVarOrc, (ms, v) => ms + v)
                .ToList();
