   ...
   public class CilECService : BaseService
    {
        ...
        public async Task<PainelCILeECResponseDto> GetCileEC(
            int accountPlanId,
            int year,
            int? groupId = null,
            int? companyId = null,
        ...
        public async Task<PainelCILeECComparativoResponseDto> GetCileECComparativo(
            int accountPlanId,
            int year,
            int? groupId = null,
            int? companyId = null,
            int? subCompanyId = null)
        {
            ...
            if (painelAtivoRealizado == null || painelAtivoOrcado == null || painelPassivoRealizado == null || painelPassivoOrcado == null)
                return new PainelCILeECComparativoResponseDto();

            var lista = new List<CILeECResponseComparativoDto>();

            foreach (var monthAtivo in painelAtivoRealizado.Months.OrderBy(m => m.DateMonth))
            {
                var monthPassivo = painelPassivoRealizado.Months.FirstOrDefault(m => m.DateMonth == monthAtivo.DateMonth);
                ...
                // ---------- Cálculos Realizado ----------
                decimal disponibilidadeR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Financeiro")?.TotalValue ?? 0;
                decimal clientesR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Clientes")?.TotalValue ?? 0;
                decimal estoqueR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Estoques")?.TotalValue ?? 0;
                decimal outrosAtivosR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Outros Ativos Operacionais Total")?.TotalValue ?? 0;

                decimal fornecedoresR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Fornecedores")?.TotalValue ?? 0;
                decimal obrigacoesR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Obrigações Tributárias e Trabalhistas")?.TotalValue ?? 0;
                decimal outrosPassivosR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Outros Passivos Operacionais Total")?.TotalValue ?? 0;

                decimal valorAtivoOperacionalR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Operacional")?.TotalValue ?? 0;
                decimal valorPassivoOperacionalR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Operacional")?.TotalValue ?? 0;
                decimal ncgR = valorAtivoOperacionalR - valorPassivoOperacionalR;

                decimal realizavelLP_R = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Não Circulante")?.TotalValue ?? 0;
                decimal exigivelLP_R = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Não Circulante Operacional")?.TotalValue ?? 0;
                decimal ativosFixosR = monthAtivo?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Fixo")?.TotalValue ?? 0;

                decimal capitalInvestidoLiquidoR = disponibilidadeR + ncgR + realizavelLP_R - exigivelLP_R + ativosFixosR;

                decimal emprestimosR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Empréstimos e Financiamentos")?.TotalValue ?? 0;
                decimal exigivelLPFinanceiroR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Não Circulante Financeiro")?.TotalValue ?? 0;
                decimal patrimonioLiquidoR = monthPassivo?.Totalizer.FirstOrDefault(t => t.Name == "Patrimônio Liquido")?.TotalValue ?? 0;
                decimal posicaoFinanceiraTerceirosR = emprestimosR + exigivelLPFinanceiroR;
                decimal estruturaDeCapitalR = patrimonioLiquidoR + posicaoFinanceiraTerceirosR;

                var realizadoCIL = new CILResponseDto
                {
                    Name = "Capital Investido Líquido",
                    Disponibilidades = disponibilidadeR,
                    Clientes = clientesR,
                    Estoques = estoqueR,
                    OutrosAtivosOperacionais = outrosAtivosR,
                    Fornecedores = fornecedoresR,
                    ObrigacoesTributariasTrabalhistas = obrigacoesR,
                    OutrosPassivosOperacionais = outrosPassivosR,
                    NCG = ncgR,
                    RealizavelLongoPrazo = realizavelLP_R,
                    ExigivelALongoPrazoOperacional = exigivelLP_R,
                    AtivosFixos = ativosFixosR,
                    CapitalInvestidoLiquido = capitalInvestidoLiquidoR
                };

                var realizadoEC = new ECResponseDto
                {
                    Name = "Posição Financeira Líquida",
                    Emprestimos = emprestimosR,
                    PosicaoFinanceiraCurtoPrazo = emprestimosR,
                    ExigivelaLongoPrazoFinanceiro = exigivelLPFinanceiroR,
                    PosicaoFinanceiraTerceiros = posicaoFinanceiraTerceirosR,
                    PatrimonioLiquido = patrimonioLiquidoR,
                    EstruturaDeCapital = estruturaDeCapitalR
                };

                // ---------- Cálculos Orçado ----------
                decimal disponibilidadeO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Financeiro")?.TotalValue ?? 0;
                decimal clientesO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Clientes")?.TotalValue ?? 0;
                decimal estoqueO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Estoques")?.TotalValue ?? 0;
                decimal outrosAtivosO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Outros Ativos Operacionais Total")?.TotalValue ?? 0;

                decimal fornecedoresO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Fornecedores")?.TotalValue ?? 0;
                decimal obrigacoesO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Obrigações Tributárias e Trabalhistas")?.TotalValue ?? 0;
                decimal outrosPassivosO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Outros Passivos Operacionais Total")?.TotalValue ?? 0;

                decimal valorAtivoOperacionalO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Operacional")?.TotalValue ?? 0;
                decimal valorPassivoOperacionalO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Operacional")?.TotalValue ?? 0;
                decimal ncgO = valorAtivoOperacionalO - valorPassivoOperacionalO;

                decimal realizavelLP_O = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Não Circulante")?.TotalValue ?? 0;
                decimal exigivelLP_O = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Não Circulante Operacional")?.TotalValue ?? 0;
                decimal ativosFixosO = monthAtivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Ativo Fixo")?.TotalValue ?? 0;

                decimal capitalInvestidoLiquidoO = disponibilidadeO + ncgO + realizavelLP_O - exigivelLP_O + ativosFixosO;

                decimal emprestimosO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Empréstimos e Financiamentos")?.TotalValue ?? 0;
                decimal exigivelLPFinanceiroO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Passivo Não Circulante Financeiro")?.TotalValue ?? 0;
                decimal patrimonioLiquidoO = monthPassivoOrcado?.Totalizer.FirstOrDefault(t => t.Name == "Patrimônio Liquido")?.TotalValue ?? 0;
                decimal posicaoFinanceiraTerceirosO = emprestimosO + exigivelLPFinanceiroO;
                decimal estruturaDeCapitalO = patrimonioLiquidoO + posicaoFinanceiraTerceirosO;

                var orcadoCIL = new CILResponseDto
                {
                    Name = "Capital Investido Líquido",
                    Disponibilidades = disponibilidadeO,
                    Clientes = clientesO,
                    Estoques = estoqueO,
                    OutrosAtivosOperacionais = outrosAtivosO,
                    Fornecedores = fornecedoresO,
                    ObrigacoesTributariasTrabalhistas = obrigacoesO,
                    OutrosPassivosOperacionais = outrosPassivosO,
                    NCG = ncgO,
                    RealizavelLongoPrazo = realizavelLP_O,
                    ExigivelALongoPrazoOperacional = exigivelLP_O,
                    AtivosFixos = ativosFixosO,
                    CapitalInvestidoLiquido = capitalInvestidoLiquidoO
                };

                var orcadoEC = new ECResponseDto
                {
                    Name = "Posição Financeira Líquida",
                    Emprestimos = emprestimosO,
                    PosicaoFinanceiraCurtoPrazo = emprestimosO,
                    ExigivelaLongoPrazoFinanceiro = exigivelLPFinanceiroO,
                    PosicaoFinanceiraTerceiros = posicaoFinanceiraTerceirosO,
                    PatrimonioLiquido = patrimonioLiquidoO,
                    EstruturaDeCapital = estruturaDeCapitalO
                };

                // ---------- Variação ----------
                var variacaoCIL = new CILResponseDto
                {
                    Name = "Capital Investido Líquido",
                    Disponibilidades = disponibilidadeR - disponibilidadeO,
                    Clientes = clientesR - clientesO,
                    Estoques = estoqueR - estoqueO,
                    OutrosAtivosOperacionais = outrosAtivosR - outrosAtivosO,
                    Fornecedores = fornecedoresR - fornecedoresO,
                    ObrigacoesTributariasTrabalhistas = obrigacoesR - obrigacoesO,
                    OutrosPassivosOperacionais = outrosPassivosR - outrosPassivosO,
                    NCG = ncgR - ncgO,
                    RealizavelLongoPrazo = realizavelLP_R - realizavelLP_O,
                    ExigivelALongoPrazoOperacional = exigivelLP_R - exigivelLP_O,
                    AtivosFixos = ativosFixosR - ativosFixosO,
                    CapitalInvestidoLiquido = capitalInvestidoLiquidoR - capitalInvestidoLiquidoO
                };

                var variacaoEC = new ECResponseDto
                {
                    Name = "Posição Financeira Líquida",
                    Emprestimos = emprestimosR - emprestimosO,
                    PosicaoFinanceiraCurtoPrazo = emprestimosR - emprestimosO,
                    ExigivelaLongoPrazoFinanceiro = exigivelLPFinanceiroR - exigivelLPFinanceiroO,
                    PosicaoFinanceiraTerceiros = posicaoFinanceiraTerceirosR - posicaoFinanceiraTerceirosO,
                    PatrimonioLiquido = patrimonioLiquidoR - patrimonioLiquidoO,
                    EstruturaDeCapital = estruturaDeCapitalR - estruturaDeCapitalO
                };

                lista.Add(new CILeECResponseComparativoDto
                {
                    Name = monthAtivo.Name,
                    DateMonth = monthAtivo.DateMonth,
                    RealizadoCIL = realizadoCIL,
                    OrcadoCIL = orcadoCIL,
                    VariacaoCIL = variacaoCIL,
                    RealizadoEC = realizadoEC,
                    OrcadoEC = orcadoEC,
