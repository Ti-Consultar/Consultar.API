    ...
    public class CashFlowService : BaseService
    {
        ...
        public async Task<PainelCashFlowResponseDto> GetCashFlow(int accountPlanId, int year)
        {
            ...
            foreach (var monthAtivo in painelAtivo.Months.OrderBy(m => m.DateMonth))
            {
                ...
                // 🔹 Calcular todas as variáveis do mês (igual seu código)
                ...
                decimal variacaoDepreciacaoAmortAcumulada = (monthAtivo.Totalizer.FirstOrDefault(t => t.Name == "Depreciação / Amort. Acumulada")?.TotalValue ?? 0) - depreciacaoAnterior;
                ...
                decimal fluxoCaixaOperacional = variacaoNCG + variacaoDepreciacaoAmortAcumulada;
